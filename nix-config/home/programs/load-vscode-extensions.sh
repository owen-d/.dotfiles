...
# This has been adjusted from the nixpkgs script:
# https://github.com/NixOS/nixpkgs/blob/master/pkgs/misc/vscode-extensions/update_installed_exts.sh
# First, run `code --extensions-dir <scratch>` and install any relevant extensions
# Then, run this script as `<path-to-script> 'code --extensions-dir <scratch>' 2>/dev/null > <extensions-file>` and it will output the required metadata.
# I hate bleeding on the edge :(
